 ...
 * 2. Download: Sử dụng expo-file-system thay vì axios cho file lớn
 * 
...
/**
 * Upload một file
 * @param {Object} file - Object file từ DocumentPicker { uri, name, mimeType }
 ...
 * @param {FormData} formData - FormData đã được append sẵn { uri, name, type }
 * @returns {Promise} Response
 */
...
/**
 * Upload cả folder
 * LƯU Ý: Mobile thường không hỗ trợ chọn cả folder như Web
 ...
 * LƯU Ý: Trên Mobile nên dùng expo-file-system để tải trực tiếp
 * @param {string} id - ID file
...
/**
 * Lấy URL download để dùng với expo-file-system
 * @param {string} id - ID file
 * @returns {string} URL download
 */
const getDownloadUrl = (id) => {
    return `${getBaseUrl()}/files/download/${id}`;
};

/**
 * Xem trước file (lấy Blob để hiển thị)
 * @param {string} id - ID file
 ...
 * @param {string} [params.fileType] - Loại file
 * @param {string} [params.ownerId] - ID người tạo
 * @param {string} [params.locationId] - ID folder chứa
 * @param {string} [params.fromDate] - Ngày bắt đầu
 * @param {string} [params.toDate] - Ngày kết thúc
 ...
 * @param {Object} [config] - Cấu hình axios (signal, etc.)
 * @returns {Promise} Response ch��a kết quả tìm kiếm
 */
...
/**
 * Tìm kiếm ngữ nghĩa (AI Semantic Search)
 * @param {string} query - Câu truy vấn mô tả
...
/**
 * Tìm kiếm bằng hình ảnh (AI Image Search)
 * @param {Object} imageAsset - Asset từ ImagePicker { uri, name, mimeType }
 * @param {number} limit - Số lượng kết quả tối đa
 * @returns {Promise} Response
 */
const searchByImage = async (file, limit = 20) => {
    const formData = new FormData();
    
    // Append file với đúng format cho React Native
    formData.append('file', {
        uri: file.uri,
...
/**
 * Lấy ảnh của một trang (trả về Blob)
 * LƯU Ý: Mobile cần convert Blob -> Base64 để hiển thị
...
/**
 * Lấy URL ảnh trang để dùng với Image component
 * @param {string} pageId - ID trang
 * @returns {string} URL ảnh
 */
const getPageImageUrl = (pageId) => {
 ...
 * @param {string|null} parentId - ID folder cha trong trash
...
/**
 * Xóa vĩnh viễn files (Hard Delete)
